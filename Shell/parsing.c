...
static bool parse_redir_flow(struct execcmd* c, char* arg) {
	...
	if ((outIdx = block_contains(arg, '>')) >= 0) {
		...
		switch (outIdx) {
			// stdout redir
			case 0: {
				strcpy(c->out_file, arg + append);
				c->out_file_mode = append;
				break;
			}
			// stderr redir
			case 1: {

				strcpy(c->err_file, arg + outIdx + append);
				c->err_file_mode = append;

				if (arg[0] == '&'){
					// &>
					strcpy(c->out_file, arg + outIdx + append);
					c->out_file_mode = append;
				}
...
static char* expand_environ_var(char* arg) {
	...
	if (arg[0] == '$'){
		...
		if (!env_var){
			...
			size_t len_var = strlen(env_var);
			for (int i = 0; i < len_var; i++){
				arg[i] = env_var[i];
			}
			arg[len_var] = END_STRING;
		}
	}
